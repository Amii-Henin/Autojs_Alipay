...
function main(){
    ...
    for (var s = 0; s < 2333; s += 1){
        switch (stole_energy(">>>>>>【【先进入谁的支付宝】】<<<<<<",chk)){
            case 1:
                if (new Date().getHours() != 14 && new Date().getHours() != 15){ 
                    s = 6666;
                    break;
                }
                // sleep(2000);
            ...
            case 4:
                ...
                toast_log("切换至奇遇偷能量啦~~");
                switch_account(account_list[1]);
                stole_energy(">>>>>>【【要浇水给谁】】<<<<<<",chk);
                switch_account(account_list[0]);
                s = 6666;
                break;
        }
        if (new Date().getHours() == 7 && (new Date().getMinutes() >= 40)){ s = 6666; }
        // sleep(3000);
    }
    remove_app("支付宝");
    toast_log ("🐱🐱🐱Done, 美滋滋~🐱🐱🐱");
